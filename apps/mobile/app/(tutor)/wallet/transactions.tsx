...
export default function TransactionsScreen() {
  ...
  const isIncome = (tx: TransactionResponse) => {
    return tx.type === 'SESSION_PAYMENT';
  };

  const filteredTransactions = transactions.filter(tx => {
    if (filter === 'all') return true;
    if (filter === 'income') return isIncome(tx);
    if (filter === 'expense') return !isIncome(tx);
    return true;
  });
              ...
              <View style={styles.transactionHeader}>
                ...
                <View style={styles.transactionRight}>
                  <Text style={[
                    ...
                    { color: isIncome(tx) ? Colors.success : Colors.error }
                  ]}>
                    {isIncome(tx) ? '+' : '-'}{formatCurrency(tx.amount)}
                  </Text>
                  {isIncome(tx) && (
