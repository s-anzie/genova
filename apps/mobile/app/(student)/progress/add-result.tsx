...
export default function AddResultScreen() {
  ...
  const { goalId, subject: goalSubject } = useLocalSearchParams<{
    goalId?: string;
    subject?: string;
  }>();
  const { addResult } = useProgress();

  // Pre-fill subject if coming from a goal
  const [subject, setSubject] = useState(goalSubject || '');
  ...
  // Check if subject is locked (coming from a goal)
  const isSubjectLocked = !!goalId && !!goalSubject;
      ...
      >
        <View style={styles.form}>
          ...
          <View style={styles.formGroup}>
            ...
            <TextInput
              style={[styles.input, isSubjectLocked && styles.inputDisabled]}
              value={subject}
              onChangeText={setSubject}
              placeholder="Ex: Mathématiques"
              placeholderTextColor={Colors.textTertiary}
              editable={!isSubjectLocked}
            />
            {isSubjectLocked && (
              <Text style={styles.helperText}>
                La matière est définie par l'objectif sélectionné
              </Text>
            )}
          </View>
